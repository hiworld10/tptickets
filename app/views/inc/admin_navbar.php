<div id="cssmenu" class="__full-height-perc">
    <ul>
        <li><a href="<?=FRONT_ROOT?>/artists" >Artistas</a></li>
        <li><a href="<?=FRONT_ROOT?>/categories"  title="...">Categorias</a></li>
        <li><a href="<?=FRONT_ROOT?>/events" title="...">Eventos</a></li>
        <li><a href="<?=FRONT_ROOT?>/calendars" title="...">Calendario</a></li>
        <li><a href="<?=FRONT_ROOT?>/event-seats" title="...">Plazas</a></li>
        <li><a href="<?=FRONT_ROOT?>/seat-types" title="...">Tipo Plazas</a></li>
        <li><a href="<?=FRONT_ROOT?>/place-events" title="...">Lugar</a></li>
        <li><a href="<?=FRONT_ROOT?>/users" title="...">Usuarios</a></li>
    </ul>
</div>